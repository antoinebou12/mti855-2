    ...
    mesh(nullptr)
{
    ...
    //if (_geometry->getType() == kConvexHull)
    //{
    //    ConvexHull* ch = dynamic_cast<ConvexHull*>(_geometry);
    //    const unsigned int nverts = ch->points.size();
    //    const unsigned int nfaces = ch->faces.size();
    //    Eigen::MatrixXf meshV(nverts, 3);
    //    for (unsigned int i = 0; i < nverts; ++i)
    //    {
    //        meshV.row(i) = ch->points[i];
    //    }
    //    Eigen::MatrixXi meshF(nfaces, 3);
    //    for (unsigned int i = 0; i < nfaces; ++i)
    //    {
    //        meshF.row(i) = ch->faces[i];
    //    }

    //    polyscope::registerSurfaceMesh("convex", meshV, meshF);
    //}
